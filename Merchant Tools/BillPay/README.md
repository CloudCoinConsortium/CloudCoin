...
CloudCoin maybe the only currency in the world that can be distributed without fees. The cost of sending Bitcoin and other cryptocurrencies, to massive amounts of users can be extremely expensive. For most payment systems, it is less expensive to send cash via the plain old mail than electronically. The only advantage of payment systems over regular mail is the speed, accountability, and dependability. With CloudCoin, you can pay thousands of people within minutes and pay no fees. You also have the addition of privacy which is not possible with most payment systems such as PayPal, Western Union, Bank Transfer, and others.

If you want to be able to get CloudCoins out to a lot of fo people quickly, use the CloudCoin Wallet's Bill Pay.
To get the CloudCoin Wallet's Bill Pay to work, you will open your CloudCoin Wallet and then go to Tools>BillPay The program will then ask you to choose what wallet you want to send money from. Then you will be asked to load a CSV (Comma Separated Value) file. This "CSV" file will specify who you want to send money too, how much to send, and which transport system to use. This CSV file can be created and edited in a spreadsheet program such as Microsoft Excel, Google Sheets, Open Office Sheets, or any other spreadsheet software). You can also just use a text editor.
...
Requirements: 
1. A professional subscription to ProtonMail
2. Download the ProtonBridge
3. You can only send 150 emails each day. 
4. Create the email template file
5. Create the email configuration file
6. Make the email list CSV file.
7. Start and monitor the emailing. 
...
CSV means comma-separated values. It can be opened in any spreadsheet program such as google sheets and Microsoft Excel. 
The mailing list can be located anywhere on your computer, cloud drives
...
Sample of tile contents (Scheduled to change in future editions)
```
...
Formats used in Bill Pay include:
...
#### 25s 
The number of 25 notes CloudCoins to be sent.
...
2. For Send and Transfer, use a sky Wallet account ID
...
3. Tag: For jpegs or stack files that are put on a file system
